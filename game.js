...
scene("game", () => {
    ...
    function wizard(damage, speed, attackspeed, attackdist, bulletspeed) {
        let tweenCur = null
        let lastattack = time()
        return {
            id: "wizard",
            ...
            update() {
                if (this.pos.dist(player.pos < attackdist)) {
                    if (!(time() - lastattack > attackspeed)) return
                    //spawn bullet
                    lastattack = time()
                    let dir = player.pos.sub(this.pos).unit()
                    let bullet = add([
                        pos(this.pos),
                        rect(4, 4),
                        color(RED),
